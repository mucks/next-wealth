...
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "wealth_history_user_id_idx" ON "wealth_history" USING btree ("user_id");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "wealth_history_date_idx" ON "wealth_history" USING btree ("date");
--> statement-breakpoint
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'wealth_history' AND policyname = 'Users can view own wealth history'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'wealth_history' AND policyname = 'Users can insert own wealth history'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'wealth_history' AND policyname = 'Users can update own wealth history'
  ) THEN
